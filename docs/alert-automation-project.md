...
We do not recommend it as the v1 default, for three reasons. **(1) Latency on the dominant case.** ~90% of alert workflows are "post a Slack message, maybe call one HTTP endpoint, optionally open a ticket" — sub-second end-to-end. An Argo container-per-step adds a 5–15 s pod-startup tax we would inherit on every alert; an Airflow operator pays a scheduler tick. We would degrade the headline alert-to-action latency by an order of magnitude to buy scale we do not yet need. **(2) Keep stays on the front anyway.** Argo is submit-and-wait batch; Airflow is interval-scheduled. Neither has a native "match this CEL against every incoming alert and fan out workflows" model — so Keep would still own ingestion, CEL matching, dedup, severity-changed/only-on-change gating and fingerprint dedup, and would RPC into Argo/Airflow per match. We add a network hop without removing complexity. **(3) The scale path stays inside Keep.** Keep ships ARQ (Redis-backed distributed queue) plumbing in `src/common/arq_pool.py` and on the alert-ingestion path. The workflow-execution side, however, is **not yet a flag flip**: the ARQ handler at `src/workflowmanager/workflowmanager.py:522-568` is currently a commented-out `TODO`. Finishing it is in-source work, not a re-platform — and at our ingest rate of ~4,000 alerts/minute it is plausibly **part of v1**, not "if/when load demands it". This work is far smaller than building a Keep→Argo or Keep→Airflow transpiler.
...
Recommendation: ship on Keep's engine for v1; **finish the ARQ-distributed workflow-execution path as part of v1** given the 4K alerts/min ingest rate; add an Argo step-executor as a hybrid escape hatch once at least one team has a concrete heavy-step or destructive-remediation need. Defer any full transpilation work until we have data on workflow shape, QPS and step durations across teams.
...
Single-node `WorkflowScheduler` polling loop (1 s tick) plus a fixed `ThreadPoolExecutor` (20 workers, `src/workflowmanager/workflowscheduler.py:649`). In-memory queue (`workflows_to_run`, lock-protected). ARQ (Redis-backed) plumbing already exists in `src/common/arq_pool.py` and the alert-ingestion path (`src/common/event_management/process_event_task.py`); the workflow-execution side is a commented-out `TODO` at `src/workflowmanager/workflowmanager.py:522-568` — connection plumbing in place, worker entrypoint unfinished. **Trigger-matching throughput.** `WorkflowManager.insert_events()` evaluates every workflow's CEL against every incoming alert. At our ingest rate of ~4,000 alerts/min × N workflows per tenant, the matching pass — not workflow execution — is the first place the single-node scheduler could become a bottleneck; this is the single most important number to validate empirically during discovery.
...
| Option | Trade-off |
|---|---|
| **Keep alone (current)** | Sub-second alert-to-action latency. Full provider catalog, multi-tenant, dedup/enrichment first-class. Single-node executor today; ARQ-distributed workflow execution is a commented-out `TODO` in source — finishing it is in-source work, plausibly part of v1 at 4K alerts/min ingest. |
