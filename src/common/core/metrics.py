...
# Must be settled before prometheus_client is imported: multiprocess mode is
# selected at import time based on this env var. (Previously the import sat
# above this block, so under gunicorn the metrics silently fell back to
# single-process mode and never reached the dir the /metrics endpoint scrapes.)
