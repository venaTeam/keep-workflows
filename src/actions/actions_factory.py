...
from src.actions.actions_exception import ActionsCRUDException
...
from src.common.models.action import ActionDTO
...
class ActionsCRUD:
    ...
    def _convert_models_to_dtos(models: List[Action]) -> List[ActionDTO]:
        ...
        for model in models:
            try:
                ...
                logger.warning(
                    "Unmatched Action model and the coresponding DTO",
                    exc_info=True,
        ...
        try:
            ...
            for action_dto in action_dtos:
                action = Action(
                    ...
                    use=action_dto.get("use")
                    or action_dto.get(
                        "name"
                    ),  # if there is no `use` tag, use `name` instead
            ...
            raise ActionsCRUDException(
                status_code=422, detail="Unable to create the actions"
            )
    ...
    def remove_action(tenant_id: str, action_id: str):
        try:
            ...
            raise ActionsCRUDException(
                status_code=422, detail="Unable to delete the requested action"
            )
    ...
    def get_action(tenant_id: str, action_id: str) -> Union[Action, None]:
        try:
            ...
            raise ActionsCRUDException(
                status_code=400, detail="Unable to get an action"
            )

    @staticmethod
    def update_action(
        tenant_id: str, action_id: str, payload: dict
    ) -> Union[Action, None]:
        try:
            ...
            raise ActionsCRUDException(
                status_code=422, detail="No action matched to be updated"
            )
        except Exception:
            logger.exception("Uknown exception when update an action on database")
            raise ActionsCRUDException(
                status_code=400, detail="Unable to update an action"
            )
