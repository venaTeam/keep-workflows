...
from src.common.models.db.workflow import Workflow
...
def test_severity_greater_than_info_bug_fix(
    ...
    critical_alert = create_alert(
        severity=AlertSeverity.CRITICAL, fingerprint="fp-critical"
    )
    warning_alert = create_alert(
        severity=AlertSeverity.WARNING, fingerprint="fp-warning"
    )
...
def test_severity_greater_than_or_equal_warning(
    ...
    for alert in [critical_alert, high_alert, warning_alert]:
        ...
        assert (
            len(workflow_manager.scheduler.workflows_to_run)
            == workflows_to_run_before + 1
        )
    ...
    for alert in [info_alert, low_alert]:
        ...
        workflow_manager.insert_events(SINGLE_TENANT_UUID, [alert])
        assert (
            len(workflow_manager.scheduler.workflows_to_run) == workflows_to_run_before
        )
...
def test_case_insensitive_severity_comparisons(
    ...
    create_workflow("test-severity-case", "severity > 'INFO'")
...
def test_severity_preprocessing_cel_utils_integration(
    ...
    workflow = create_workflow("test-preprocessing-integration", "severity > 'info'")
    ...
    high_alert = create_alert(
        severity=AlertSeverity.HIGH, source=["test"], fingerprint="fp-high-severity"
    )
